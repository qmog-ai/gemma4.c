...
Model *load_model(const char *path, size_t *size) {
    ...
    struct stat status;
    if (fd < 0 || fstat(fd, &status)) { perror(path); exit(1); }
    *size = (size_t)status.st_size;
    Model *model = mmap(NULL, *size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
    close(fd);
    if (model == MAP_FAILED) { perror("mmap"); exit(1); }
    if (memcmp(model->magic, "MOF", 4)) { fprintf(stderr, "bad model file\n"); exit(1); }
    ...
    for (size_t i = 0; i < sizeof(model->weights) / sizeof(*tensors); i++) {
        ...
        tensors[i].scales = tensors[i].scales ? (float *)((uint8_t *)model + (uintptr_t)tensors[i].scales) : NULL;
    }
    return model;
}
    ...
    for (size_t token = 0; token < token_count; token++) {
        ...
        for (size_t weight_row = 0; weight_row < outputs; weight_row++) {
            ...
            for (size_t group = 0; group < groups; group++) {
                ...
                int32_t dot = dot_i8(input_row + offset, row_weights + offset);
                sum += (float)dot * scales[group] * weight_scales[group];
            }
            result[token * outputs + weight_row] = sum;
        }
    }
}
...
void quantize(int8_t *quantized, float *scales, const float *input, size_t rows, size_t width) {
    for (size_t group = 0; group < rows * (width / 64); group++) {
        ...
        float max_abs = 0.0f;
        for (int j = 0; j < 64; j++) {
            float value = fabsf(values[j]);
            if (value > max_abs) max_abs = value;
        }
        float scale = max_abs / 127.0f;
        float inverse_scale = scale > 0.0f ? 1.0f / scale : 0.0f;
        for (int j = 0; j < 64; j++) quantized[group * 64 + j] = (int8_t)rintf(values[j] * inverse_scale);
        scales[group] = scale;
    }
}
...
// Looks up int8 embedding rows and dequantizes them directly.
void embedding(float *output, const Tensor *table, const int *tokens, size_t token_count, float multiplier) {
    ...
    for (size_t token = 0; token < token_count; token++) {
        ...
        const float *scales = table->scales + (size_t)tokens[token] * groups;
        for (size_t group = 0; group < (size_t)groups; group++) {
            float scale = scales[group] * multiplier;
            for (int j = 0; j < 64; j++)
                vector[group * 64 + j] = (float)row[group * 64 + j] * scale;
        }
    }
}
...
void attention(InferenceState *state, const LayerWeights *layers, int layer,
    ...
    int cache_layer = layer;
    while (!layers[cache_layer].k_proj.data || (cache_layer % 5 == 4) != is_full_attention) cache_layer--; // Shared layers reuse the latest cache of the same attention type.
    float *key_cache = is_full_attention ? state->full_cache[cache_layer / 5] : state->sliding_cache[cache_layer / 5][cache_layer % 5];
    float *value_cache = key_cache + (size_t)cache_size * head_dim;
    ...
    for (size_t head = 0; head < (size_t)(query_width / head_dim); head++) {
        for (size_t token = 0; token < token_count; token++) {
            ...
            int key_count = start_pos + (int)token + 1 - first_key;
            ...
            for (int key_offset = 0; key_offset < key_count; key_offset++) {
                const float *key = key_cache + ((first_key + key_offset) & cache_mask) * head_dim;
                scores[key_offset] = dot_f32(query, key, head_dim);
            }
            softmax(scores, key_count);
            weighted_sum(head_output, scores, value_cache, first_key, key_count, cache_mask, head_dim);
        }
    }
...
float random_uniform(void) {
    ...
    return (float)((rng_state * UINT64_C(0x2545F4914F6CDD1D)) >> 40) / 16777216.0f;
}

int sample(const float *scores, float temperature) {
    if (temperature <= 0.0f) return greedy(scores);
    ...
    float sum = 0.0f;
    float maximum = top[0].score / temperature;
    for (int i = 0; i < 64; i++)
        sum += top[i].score = expf(top[i].score / temperature - maximum);
